...
void setNodeTypes(AST *node)
{
    ...
    else if (isArithmeticOperation(node->type))
    {
        ...
        fprintf(stderr, "s1:%d -s2: %d", son0->datatype, son1->datatype);
...
int isNumber(AST *son)
{
    ...
    if (
        ...
        || (son->type == AST_SYMBOL && 
        ((son->symbol->type == SYMBOL_LIT_INTEGER || 
        ((son->symbol->type == SYMBOL_VARIABLE || son->symbol->type == SYMBOL_PARAMETER) && isInteger(son->symbol->datatype))) 
